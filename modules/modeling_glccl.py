...
class GLCCL(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        self.word_frame_logit_weight = nn.parameter.Parameter(torch.eye(num_words), requires_grad=True)

        # for fine-grained constrast weights
        self.local_mat_weight = nn.parameter.Parameter(torch.eye(embed_dim), requires_grad=True)
        self.word_frame_mat_weight = nn.parameter.Parameter(torch.eye(num_words), requires_grad=True)
        self.word_mat_weight = nn.parameter.Parameter(torch.eye(num_words), requires_grad=True)
        self.word_frame_mat_weight2 = nn.parameter.Parameter(torch.eye(num_words), requires_grad=True)
    ...
    def _loose_similarity(self, sequence_output, seq_features, visual_output, attention_mask, video_mask, sim_header="meanP"):
        ...
        if self.interaction_type == 'attn':
            sentence_frame_sim_score = torch.einsum('ad, abtd -> abt', [sentence_output, frame_features]) # bs_text dim, bs_text bs_video num_words dim -> bs_text bs_video num_words
            sentence_frame_weights = torch.matmul(torch.softmax(sentence_frame_sim_score / 1e-2, dim=-1), self.word_frame_logit_weight)
    ...
    def fine_grained_interaction(self, word_features, frame_features):
        ...
        bs_text, bs_video, num_words, dim = frame_features.shape

        word_features_ = torch.matmul(word_features, self.local_mat_weight) # bs_text num_words dim, dim dim -> bs_text num_words dim
        fine_grained_sim_scores = torch.einsum('atd, abvd -> atbv', [word_features_, frame_features]) # bs_text num_words dim, bs_tetx bs_video num_words dim -> bs_text num_words bs_video num_words
       
        if self.interaction_type == 'attn':
            # bs_text num_words bs_video num_words -> bs_text bs_video num_words num_words -> bs_text num_words bs_video num_words -> bs_text bs_video num_words
            ...
            # bs_text num_words bs_video num_words -> bs_text num_words bs_video num_words -> bs_text num_words bs_video 
            frame_level_logit = torch.sum(torch.matmul(torch.softmax(fine_grained_sim_scores/1e-2, dim=-1), self.word_frame_mat_weight) * fine_grained_sim_scores, dim=-1)                                             
            # bs_text bs_video num_words -> bs_text bs_video num_words -> bs_text bs_video
            sent2frame_logits = torch.sum(torch.matmul(torch.softmax(word_level_logit/1e-2, dim=-1), self.word_frame_mat_weight2) * word_level_logit, dim=-1) 
