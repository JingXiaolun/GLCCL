...
class XCLIP(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        self.visual_logit_weight = nn.parameter.Parameter(torch.eye(num_frames), requires_grad=True)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None):
        ...
        if self.training:
            loss = 0.
            sim_matrix, sim_matrix_list = self.get_similarity_logits(sequence_output, seq_features, visual_output, attention_mask, 
            ...
            if self.var_loss_flag:
                ...
                sim_matrix_var_tem = sim_matrix_var.clone()
    ...
    def _loose_similarity(self, sequence_output, seq_features, visual_output, attention_mask, video_mask, sim_header="meanP"):
        ...
        # text_guided allgather operation
        if self.training:
            sequence_output = allgather(sequence_output, self.task_config)
            seq_features = allgather(seq_features, self.task_config)
        ...
        if self.text_guided_flag:
            # [bs, 1 + num_words, dim]
            cls = sequence_output.squeeze(1)
            video_feat = visual_output

            if self.aggregation_weights_type == 'softmax':
                v_weight = torch.einsum('ad, bvd -> abv', [cls, video_feat]) # bs_text dim, bs_video num_frames dim -> bs_text bs_video num_frames 
                v_weight = torch.softmax(v_weight / 5 , dim=-1)
                v_weight = torch.einsum('abv,bv->abv', [v_weight, video_mask]) 
                video_output = torch.einsum('abv, bvd -> abd', [v_weight, video_feat]) # bs_text bs_video num_frames, bs_video num_frames dim -> bs_text bs_video dim

            # [bs_text, bs_video, dim]
        ...
        sentence_output_ = torch.matmul(sentence_output, self.global_mat_weight) # bs_text dim, dim dim -> bs_text dim
        video_sentence_logits = torch.einsum('ad, abd -> ab', [sentence_output_, video_output])
        video_sentence_logits *= logit_scale

        # video-word score
        if self.interaction_type == 'attn':
            video_word_sim_score = torch.einsum('abd, atd -> atb', [video_output, word_features]) # bs_text bs_video dim, bs_text num_words dim -> bs_text num_words bs_video
            #print(f'shape of video_word_sim_score: {video_word_sim_score.shape}')
            video_word_weights = torch.matmul(torch.softmax(video_word_sim_score / 1e-2, dim=1).permute(0,2,1), self.word_logit_weight).permute(0,2,1) # bs_text bs_video num_words -> bs_text num_words bs_video
            video_word_logits = logit_scale * torch.sum(video_word_sim_score * video_word_weights, dim=1) # bs_text num_words bs_video, bs_text num_words bs_video -> bs_text bs_video

        # sentence-frame score 
        if self.interaction_type == 'attn':
            sentence_frame_sim_score = torch.einsum('ad, bvd -> abv', [sentence_output, frame_features]) # bs_text dim, bs_text bs_video num_words dim -> bs_text bs_video num_words
            #print(f'shape of sentence_frame_sim_score: {sentence_frame_sim_score.shape}')
            sentence_frame_weights = torch.matmul(torch.softmax(sentence_frame_sim_score / 1e-2, dim=-1), self.frame_logit_weight)
            sentence_frame_logits = logit_scale * torch.sum(sentence_frame_sim_score * sentence_frame_weights, dim=-1) # bs_text bs_video num_words, bs_text bs_video  num_words -> bs_text bs_video

        ## frame-word score
        frame_word_logits = logit_scale * self.fine_grained_interaction(word_features, frame_features)

        logits = (video_sentence_logits + video_word_logits + sentence_frame_logits + frame_word_logits) / 4

        logits_list = [video_sentence_logits, video_word_logits, sentence_frame_logits, frame_word_logits]

        return logits, logits_list
   
    def fine_grained_interaction(self, word_features, frame_features):
        bs_text, num_words, dim = word_features.shape
        bs_video, num_frames, dim = frame_features.shape

        word_features_ = torch.matmul(word_features, self.local_mat_weight) # bs_text num_words dim, dim dim -> bs_text num_words dim
        fine_grained_sim_scores = torch.einsum('atd, bvd -> atbv', [word_features_, frame_features]) # bs_text num_words dim, bs_video num_frames dim -> bs_text num_words bs_video num_frames
       
        if self.interaction_type == 'attn':
            # bs_text num_words bs_video num_frames -> bs_text bs_video num_frames num_words -> bs_text num_words bs_video num_frames -> bs_text bs_video num_frames
            word_level_logit = torch.sum(torch.matmul(torch.softmax(fine_grained_sim_scores/1e-2, dim=1).permute(0,2,3,1), self.word_mat_weight).permute(0,3,1,2) * fine_grained_sim_scores, dim=1)                   
            # bs_text num_words bs_video num_frames -> bs_text num_words bs_video num_frames -> bs_text num_words bs_video 
            frame_level_logit = torch.sum(torch.matmul(torch.softmax(fine_grained_sim_scores/1e-2, dim=-1), self.frame_mat_weight) * fine_grained_sim_scores, dim=-1)                                             
            # bs_text bs_video num_frames -> bs_text bs_video num_frames -> bs_text bs_video
            sent2frame_logits = torch.sum(torch.matmul(torch.softmax(word_level_logit/1e-2, dim=-1), self.frame_mat_weight2) * word_level_logit, dim=-1) 
            # bs_text num_words bs_video -> bs_text bs_video num_words -> bs_text num_words bs_video -> bs_text bs_video
            video2word_logits = torch.sum(torch.matmul(torch.softmax(frame_level_logit/1e-2, dim=1).permute(0,2,1), self.word_mat_weight2).permute(0,2,1) * frame_level_logit, dim=1) 
    ...
    def get_similarity_logits(self, sequence_output, seq_features, visual_output, attention_mask, video_mask, shaped=False, loose_type=False):
        ...
        if loose_type:
            ...
            retrieve_logits, retrieve_logits_list = self._loose_similarity(sequence_output, seq_features, visual_output, attention_mask, video_mask, sim_header=self.sim_header)
            return retrieve_logits, retrieve_logits_list
        else:
