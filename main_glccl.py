...
def get_args(description='GLCCL on Retrieval Task'):
    ...
    parser.add_argument('--aggregation_weights_type', type=str, default='softmax', choices=['softmax', 'softmax_linear_softmax', 'mlp_softmax'], help='type of aggregation weights')
    ...
    parser.add_argument('--interaction_type', type=str, default='attn', choices=['ti', 'wti', 'max-max', 'mean-mean', 'mean-max', 'attn', 'ISA', 'LISA_GISA', 'WISA', 'PISA'], help='cross-modal interaction at frame-word level')
    parser.add_argument('--window_size', type=int, default=2, help="window size")
    parser.add_argument('--pale_size', type=int, default=4, help="pale size")
...
def set_seed_logger(args):
    ...
    if args.local_rank == 0:
        ...
        for key in sorted(args.__dict__):
            if key in ['num_mha_heads', 'transformer_dropout']:
                continue
...
def save_model(epoch, args, model, optimizer, tr_loss, type_name=""):
    ...
    ## save all models corresponding to epochs
    ...
    # save best model (standard: text-to-video R@1)
...
def eval_epoch(epoch, args, model, test_dataloader, device, n_gpu, writer):
    ...
    # write scalars into visualize_dir
    ...
    # write metrics into logger
    logger.info("Text-to-Video:")
    logger.info('\t>>>  R@1: {:.1f} - R@5: {:.1f} - R@10: {:.1f} - Median R: {:.1f} - Mean R: {:.1f} - RSum: {:.1f}'.
    ...
    logger.info("T2V-V2T:")
    logger.info('\t>>>  SumR: {:.1f}'.format(tv_metrics['R1'] + tv_metrics['R5'] + tv_metrics['R10'] + vt_metrics['R1'] + vt_metrics['R5'] + vt_metrics['R10']))  
    ...
    if args.do_train:
        ...
        for epoch in range(resumed_epoch, args.epochs):
            ...
            if args.local_rank == 0:
                ...
                ## Run on test dataset for selecting best model.
        ...
        ### uncomment Test on the best checkpoint
        #if args.local_rank == 0:
